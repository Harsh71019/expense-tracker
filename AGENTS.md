...
- Runtime boundaries (HTTP bodies, env vars, CSV rows, queue payloads, Mongo docs leaving the repository layer) are **parsed with zod, not asserted**. `JSON.parse` result is `unknown` until validated.
...
3. **Every money write goes through `withTxn`** (MongoDB session, `snapshot` read concern, `majority` write concern). Insert + account balance `$inc` + audit entry are one transaction or nothing. No business code calls `startSession` directly.
4. **Nothing slow inside a transaction**: no HTTP calls, no file I/O, no CSV parsing inside a session. Parse first, transact last. Batch writes in chunks of ≤200 docs per transaction.
...
- **Controllers**: HTTP only — parse/validate (zod pipe), call one service method, map result. No Mongoose, no business logic.
- **Services**: business rules and transaction orchestration. No `req`/`res` types, no HTTP status decisions.
- **Repositories**: the only layer that touches Mongoose. **Every repository method takes `userId` as a required first parameter and includes it in every filter.** No exceptions, including "it's single-user anyway". Handlers never read `userId` from a request body — only from the session (`@CurrentUser()`).
...
- Notifications are written to the `notification_outbox` collection **inside the triggering transaction**, drained by a worker. Never call ntfy/Telegram directly from a service.
- New indexes, validators, or collections go in a **migrate-mongo migration**, never in application code, never manually in Atlas. Migrations are additive-only (no drops, no renames of live fields) — rollback safety depends on this.
...
- New business logic ships with tests in the same PR. Money-path code (`transactions/`, `imports/`, `common/money`, `common/mongo-txn`) has a **90% line coverage gate** — CI blocks below it.
- Integration tests run against mongodb-memory-server in **replica-set mode** (transactions must be real). Every integration/e2e test ends with `assertInvariants()` (conservation, append-only, pairing).
...
- Mongo queries are built from validated, typed values only — never spread raw query params into a filter object (NoSQL injection).
- Audit log is write-once. No code path may update or delete `audit_log` documents.
...
- Do not touch `deploy/`, `deploy.sh`, `docker-compose.yml`, or `migrations/` in the same PR as feature code unless the feature requires it — deployment changes are reviewed separately.
