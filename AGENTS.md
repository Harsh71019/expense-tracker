...
- **Repositories**: the only layer that touches Drizzle. User-facing repository methods take `userId` as a required first parameter and include it in every filter. Handlers never read `userId` from a request body — only from the session (`@CurrentUser()`).
  - The only exception is an explicitly named `system*` method used by a worker-only scheduler/sweeper that must discover work across tenants. A `system*` method may select or claim operational rows across tenants, but it must return the owning `userId`, must not update ledger/audit monetary fields, and must not be reachable from a controller or API-role service path.
  - After discovery, every tenant-scoped read or mutation receives the discovered `userId` as its first argument and filters by it. System discovery/claim methods require integration tests proving tenant ownership is preserved and a service test proving the API role cannot invoke the sweep.
