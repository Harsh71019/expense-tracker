...
- **Tokens in Tailwind v4 `@theme`** in `globals.css` — the only place colors/spacing/type are defined. Semantic, not raw: `--color-income`, `--color-expense`, `--color-reversed`, `--color-surface`, etc. Dark mode via `class` strategy + cookie (no flash: theme class applied in root layout from cookie, `<meta name="theme-color">` synced).
