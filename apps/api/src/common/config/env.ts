...
export const RuntimeEnvSchema = z
  .object({
    ...
    PORTFOLIO_IMPORT_ENCRYPTION_KEY: aes256KeySchema.optional()
  })
  .superRefine((environment, context) => {
    if (
      environment.NODE_ENV === "test" ||
      environment.PORTFOLIO_IMPORT_ENCRYPTION_KEY !== undefined
    ) {
      return;
    }
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["PORTFOLIO_IMPORT_ENCRYPTION_KEY"],
      message: "Portfolio import encryption requires a base64-encoded 32-byte AES-256 key."
    });
  });
