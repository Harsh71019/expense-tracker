...
export class GoalMutationService {
  ...
  create(userId: string, input: CreateGoal, key: string): Promise<IdempotentResult<Goal>> {
    return this.idempotency.execute(userId, "goal.create", key, GoalSchema, (tx) =>
  ...
  update(
    ...
    return this.idempotency.execute(userId, "goal.update", key, GoalSchema, (tx) =>
      this.goals.updateInTx(userId, goalId, patch, tx)
    );
  }

  abandon(userId: string, goalId: GoalId, key: string): Promise<IdempotentResult<null>> {
    return this.idempotency.execute(userId, "goal.abandon", key, z.null(), (tx) =>
      this.goals.abandonInTx(userId, goalId, tx)
    );
  }

  reorder(userId: string, input: ReorderGoals, key: string): Promise<IdempotentResult<null>> {
    return this.idempotency.execute(userId, "goal.reorder", key, z.null(), (tx) =>
