...
import { BillPaymentResultSchema, type BillPaymentResult } from "@treasury-ops/shared";
...
> {
  ...
  return useMutation({
    mutationFn: async (input): Promise<BillPaymentResult> => {
      try {
        const result = await apiClient.POST("/v1/bills/{billId}/link-payment", {
          params: { path: { billId: input.billId }, header: { "Idempotency-Key": key } },
          body: {
