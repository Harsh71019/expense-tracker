...
import { CreateTransactionSchema, parseMinor, type CreateTransaction } from "@treasury-ops/shared";
...
export function CreateTxnSheet({ onClose }: Readonly<{ onClose: () => void }>): ReactNode {
  ...
  return (
    <DialogSurface labelledBy="create-txn-title" onClose={onClose} variant="drawer">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          onClick={onClose}
          aria-label="Close"
          className="grid h-11 w-11 shrink-0 touch-manipulation place-items-center rounded-xl border border-border bg-surface-muted text-foreground-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
        >
          ✕
        </button>
      </div>
      ...
      <form onSubmit={form.handleSubmit(submit)} className="mt-6 space-y-5">
        ...
        <div className="safe-area-bottom sticky bottom-0 -mx-5 flex gap-2.5 border-t border-border bg-surface-elevated/95 px-5 pt-4 backdrop-blur sm:-mx-7 sm:px-7">
          ...
          <Button className="flex-1" type="submit" disabled={!canSubmit || create.isPending}>
