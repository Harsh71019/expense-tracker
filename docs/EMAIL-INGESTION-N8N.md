...
Bank transaction emails (ICICI, HDFC) are parsed and posted into TreasuryOps automatically via an external **n8n** workflow. This lives outside the repo (in n8n, on the home-lab box) — this doc exists so the pipeline is discoverable from the codebase.
...
Gmail (bank alert email)
  ...
  -> POST http://192.168.0.226:3006/api/v1/transactions
...
3. `last4` (the account/card's last 4 digits) is resolved to a TreasuryOps `accountId` via a hardcoded `ACCOUNT_MAP`. If the last4 isn't in the map, the row is skipped with `no_account_mapping_for_<last4>`.
...
- **Adding a new account/card**: add its last-4 digits to `ACCOUNT_MAP` in Node 1 with the account's UUID from TreasuryOps.
