...
An earlier revision of this addendum stated the narration normalizer didn't exist. That check was run against a stale worktree of this branch, forked before `feat/finance-02-narration-normalizer` (PR #121) merged into `main`. It has since merged (`c7446d3`, 2026-08-02): `apps/api/src/common/transaction-text/normalize-transaction-text.ts` + `packages/shared/src/transaction-text.ts` are real and present on `main` today. Re-verified directly against `origin/main` rather than this branch, since this branch itself is forked from a point before the full domain-module set existed (its own `apps/api/CLAUDE.md` still describes the API as "foundation stage only" — that's how stale the base is). Any implementation work coming out of this plan should branch from current `main`, not from here.
