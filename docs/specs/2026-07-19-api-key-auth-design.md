...
- Transport: `Authorization: Bearer <key>` via `customAPIKeyGetter: (ctx) => ...` (signature: `(ctx: HookEndpointContext) => string | null`, confirmed in source) that reads the `authorization` header and strips a `Bearer ` prefix, returning `null` if absent/malformed. Reuses the `req.headers.authorization` redact rule already in `app.module.ts`'s pino config — no new redaction needed — and matches the OAuth2-style bearer convention (GitHub PATs, Stripe keys, OpenAI keys all use the same scheme for opaque non-JWT tokens). Exact header-access call (`ctx.headers.get(...)` vs `ctx.request.headers.get(...)`) gets nailed down against `HookEndpointContext`'s real type during implementation — `pnpm typecheck` catches a wrong guess immediately, this isn't a behavioral risk like the ones below.
- `enableSessionForAPIKeys`: leave unset/`false`. The plugin *can* auto-convert a valid key into a transparent session (so existing `getSession`-based code paths would "just work"), but we deliberately don't use that — we need an explicit branch anyway for scope checks, rate-limit-vs-auth-failure distinction, and audit logging, and folding key-auth invisibly into session resolution would blur exactly the line `@RequireScopes()` depends on.
...
3. `customAPIKeyGetter` signature confirmed (`(ctx: HookEndpointContext) => string | null`); exact header-access call left to `pnpm typecheck` at implementation time, not a behavioral unknown.
